...
void qsort(void *v[], int left, int right, int reverse,int (*comp) (void *, void *))
{
    ...
    {
        if(!reverse) //choose increasing or decreasing order of arrangement based on reverse flag value
        {
        ...
        {
            if( (  (*comp) (v[i],v[left]) )  > 0 ) //if the value of datatype at index i is less then at partition element string
                swap(v,++last,i); //then swap the particular data type ptrs of piviot and index elements
        }
    }
    swap(v,left,last); //after array has been transversed place partition element to its appropriate index

    qsort(v,left,last-1,reverse,comp); //now apply recursive calls to further sort the array
